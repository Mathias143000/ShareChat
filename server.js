// server.js — ShareChat (multi-chat), whitelist, uploads, preview, socket.io + заголовки для Clipboard API
...
app.use((req, res, next) => {
  // Для буфера обмена и data: изображений
  res.setHeader('Permissions-Policy', "clipboard-write=(self)");
  res.setHeader('Content-Security-Policy', "default-src 'self'; img-src 'self' data: blob:; script-src 'self'; style-src 'self' 'unsafe-inline'; connect-src 'self' ws: wss:;");
...
/* ---------- static ---------- */
app.use('/public',  express.static(PUBLIC,  { maxAge: 0, fallthrough: true }));
app.use('/uploads', express.static(UPLOADS, { maxAge: 0, fallthrough: true }));
...
function getChat(idRaw) { const id = ensureChat(idRaw); return chats.get(id); }
...
app.delete('/api/chats/:id/messages', (req, res) => {
  try { const id = ensureChat(req.params.id); const c = chats.get(id); c.messages.length = 0; c.names.clear(); io.emit('chat:cleared', { id, names: [] }); return res.sendStatus(204); }
  catch (e) { console.error('clear messages error:', e); return res.status(500).json({ ok: false, error: 'internal error' }); }
});

io.on('connection', (socket) => {
  socket.emit('chats:list', { chats: sortedIds() });
  { const id = sortedIds()[0] || 1; const c  = getChat(id);
  ...
  socket.on('chat:message', (m) => {
    try{
      ...
      const image = m?.image ? String(m.image) : '';
      if (!Number.isInteger(id)) return;
      const c = getChat(id);
      const msg = image ? { name, image, time: Date.now(), id } :
                           { name, text,  time: Date.now(), id };
  ...
  socket.on('chat:clear', (payload = {}) => {
    try { const id = ensureChat(payload.id); const c  = chats.get(id); c.messages.length = 0; c.names.clear(); io.emit('chat:cleared', { id, names: [] }); }
    catch (e) { console.error('socket chat:clear error', e); }
  });
});
